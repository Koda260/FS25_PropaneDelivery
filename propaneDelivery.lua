...
function PropaneDelivery:performDeliveryManual(tankId, amount)
    local tank = self:getTankById(tankId)
    if not tank then return end

    local fillLevel = tank:getFillLevel()
    local capacity = tank:getCapacity()
    local toFill = math.min(capacity - fillLevel, amount)
    ...
    if g_currentMission:getMoney() >= totalCost then
        tank:setFillLevel(fillLevel + toFill)
        g_currentMission:addMoney(-totalCost, g_currentMission.player.farmId)
        print(string.format("Manual propane delivery: %d liters for $%.2f", toFill, totalCost))
    else
        print("Not enough money for manual propane delivery.")
    end
end
